"""
Disgust Feature Selection via Causal Circuit Analysis

Problem: The top Disgust feature shows abnormal causal dynamics - ablating
upstream components INCREASES the feature activation (+202%). This indicates
a suppressor circuit rather than an activator circuit.

Solution: Test all validated Disgust features and find one with proper
causal dynamics (positive Δ for causal components, ablation reduces activation).

Usage:
    python disgust_circuit.py --device cuda
    python disgust_circuit.py --device cpu --test

Output:
    - results/disgust_feature_comparison.csv
...
DISGUST_WORDS = ["disgusting", "revolting", "repulsive", "nauseating", "gross"]


def parse_args():
    parser = argparse.ArgumentParser(description="Disgust Feature Comparison")
...
def load_all_disgust_features(results_dir):
    """Load all validated Disgust features."""
    ...
    features = []
    for _, row in disgust_df.iterrows():
        features.append({
            "layer": int(row["layer"]),
...
def evaluate_feature(model, sae, feature_info, prompts, device):
    ...
    for prompt_data in prompts:
        ...
        baseline_logit = compute_emotion_logit(model, prompt, DISGUST_WORDS, hooks=None)

        # Ablated
        ablated_activation = compute_feature_activation(
            model, sae, prompt, feature_id, feature_layer, device, hooks=combined_hooks
        )
        ablated_logit = compute_emotion_logit(model, prompt, DISGUST_WORDS, hooks=combined_hooks)
...
def main():
    ...
    print("=" * 70)
    print("  DISGUST FEATURE COMPARISON")
    print("=" * 70)
    print(f"\n  Model: {MODEL_NAME}")
    print(f"  SAE: {GEMMA_SCOPE_RELEASE}")
    print(f"  Device: {args.device}")

    # Load all Disgust features
    print("\n  Loading all Disgust features...")
    disgust_features = load_all_disgust_features(output_dir)
    print(f"  Found {len(disgust_features)} Disgust features")

    if len(disgust_features) == 0:
        print("\n  ERROR: No validated Disgust features found!")
        print("  Run validation.py first to generate validated_features.csv")
        return

    for f in disgust_features:
    ...
    print(f"\n  Loading {n_prompts} Disgust prompts...")
    ...
    prompts = [p for p in all_prompts if p["emotion"].lower() == "disgust"]
    ...
    for idx, feature in enumerate(disgust_features):
        layer = feature["layer"]
        feature_id = feature["feature_id"]
        selectivity = feature["selectivity"]

        print(f"\n  Feature {idx + 1}/{len(disgust_features)}: L{layer}:F{feature_id} (selectivity={selectivity:.1f})")
    ...
    csv_path = os.path.join(output_dir, "disgust_feature_comparison.csv")
    ...
    print(f"\n  Best Disgust Feature: L{int(best['layer'])}:F{int(best['feature_id'])} (score={best['score']:.0f})")
    ...
    # Check if any feature has good score
    good_features = results_df[results_df["score"] >= 60]
    if len(good_features) == 0:
        print("\n  WARNING: No Disgust feature has score >= 60!")
        print("  This may indicate that Disgust features are mostly suppressors,")
        print("  or share too much overlap with Anger features.")
        print("\n  Consider:")
        print("    1. Using a different SAE width (32k, 65k, 131k)")
        print("    2. Excluding Disgust from analysis (overlap with Anger)")
        print("    3. Running discovery.py with more candidate features")
    else:
        print(f"\n  Found {len(good_features)} features with score >= 60:")
        for _, row in good_features.iterrows():
            print(f"    L{int(row['layer'])}:F{int(row['feature_id'])} (score={row['score']:.0f})")

    # Compare with current top-1 (by selectivity)
    current = disgust_features[0]  # Already sorted by selectivity
    if best["layer"] != current["layer"] or best["feature_id"] != current["feature_id"]:
        print(f"\n  NOTE: Best feature is different from top-1 by selectivity!")
    ...
    print(f"    Add DISGUST_OVERRIDE in load_validated_features():")
    print(f"    DISGUST_OVERRIDE = {{'layer': {int(best['layer'])}, 'feature_id': {int(best['feature_id'])}, 'selectivity': {best['selectivity']:.1f}}}")
