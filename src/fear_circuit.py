"""
Fear Feature Selection via Causal Circuit Analysis

Problem: The top Fear feature (L25:F3410, selectivity=14.1) shows mixed
causal dynamics - ablation reduces activation (-13.8%) but INCREASES
logit (+0.5). We want both to decrease.

Solution: Test all 11 validated Fear features and find one with proper
causal dynamics (ablation reduces both activation AND logit).

Usage:
    python fear_circuit.py --device cuda
    python fear_circuit.py --device cpu --test

Output:
    - results/fear_feature_comparison.csv
...
FEAR_WORDS = ["afraid", "scared", "fearful", "terrified"]


def parse_args():
    parser = argparse.ArgumentParser(description="Fear Feature Comparison")
...
def load_all_fear_features(results_dir):
    """Load all 11 validated Fear features."""
...
def evaluate_feature(model, sae, feature_info, prompts, device):
    ...
    for prompt_data in prompts:
        ...
        baseline_logit = compute_emotion_logit(model, prompt, FEAR_WORDS, hooks=None)

        # Ablated
        ablated_activation = compute_feature_activation(
            model, sae, prompt, feature_id, feature_layer, device, hooks=combined_hooks
        )
        ablated_logit = compute_emotion_logit(model, prompt, FEAR_WORDS, hooks=combined_hooks)
...
def main():
    ...
    print("=" * 70)
    print("  FEAR FEATURE COMPARISON")
    print("=" * 70)
    print(f"\n  Model: {MODEL_NAME}")
    print(f"  SAE: {GEMMA_SCOPE_RELEASE}")
    print(f"  Device: {args.device}")

    # Load all Fear features
    print("\n  Loading all Fear features...")
    fear_features = load_all_fear_features(output_dir)
    print(f"  Found {len(fear_features)} Fear features")

    for f in fear_features:
    ...
    print(f"\n  Loading {n_prompts} Fear prompts...")
    ...
    prompts = [p for p in all_prompts if p["emotion"].lower() == "fear"]
    ...
    for idx, feature in enumerate(fear_features):
        layer = feature["layer"]
        feature_id = feature["feature_id"]
        selectivity = feature["selectivity"]

        print(f"\n  Feature {idx + 1}/{len(fear_features)}: L{layer}:F{feature_id} (selectivity={selectivity:.1f})")
    ...
    csv_path = os.path.join(output_dir, "fear_feature_comparison.csv")
    ...
    print(f"\n  Best Fear Feature: L{int(best['layer'])}:F{int(best['feature_id'])} (score={best['score']:.0f})")
    ...
    current = fear_features[0]  # Already sorted by selectivity
    ...
    print(f"    1. Add FEAR_OVERRIDE similar to ANGER_OVERRIDE")
    print(f"    2. Use L{int(best['layer'])}:F{int(best['feature_id'])} for Fear")
